...
{
	static const float v_0 = -0.785695f;
	static const float v_1 =  0.275899f;
	static const float v_2 =  0.461940f;
	static const float v_3 =  0.785695f;
	static const float v_4 =  0.191342f;
	static const float v_5 =  0.707107f;
	static const float v_6 =  1.175876f;
	static const float v_7 =  0.353553f;
	...
	for (int i = 0; i < 8; i++)
	{
		...
		const float x_0C = v_8 * x_04 + v_1 * x_07;
		const float x_0D = v_6 * x_05 + v_3 * x_06;
		const float x_0E = v_0 * x_05 + v_6 * x_06;
		const float x_0F = v_1 * x_04 - v_8 * x_07;
		const float x_10 = v_7 * (x_0C - x_0D);
		const float x_11 = v_7 * (x_0E - x_0F);
...
inline void idct_1d_8x8(const float* src, float *out, int stridea, int strideb)
{
	static const float v_0 = -0.785695f;
	static const float v_1 =  0.275899f;
	static const float v_2 =  1.414214f;
	static const float v_3 =  1.306563f;
	static const float v_4 =  0.785695f;
	static const float v_5 = -0.275899f;
	static const float v_6 =  0.707107f;
	static const float v_7 =  0.541196f;
	static const float v_8 =  1.175876f;
	static const float v_9 =  0.500000f;
	static const float v_A =  0.250000f;
	static const float v_B =  0.353553f;
	...
	for (int i = 0; i < 8; i++)
	{
		...
		const float x_00 = v_2 * s_0;
		const float x_01 = v_C * s_1 + v_1 * s_7;
		const float x_02 = v_3 * s_2 + v_7 * s_6;
		const float x_03 = v_8 * s_3 + v_4 * s_5;
		const float x_04 = v_2 * s_4;
		const float x_05 = v_0 * s_3 + v_8 * s_5;
		const float x_06 = v_7 * s_2 - v_3 * s_6;
		const float x_07 = -v_1 * s_1 + v_C * s_7;
		const float x_09 = x_00 + x_04;
		const float x_0A = x_01 + x_03;
		const float x_0B = v_2 * x_02;
		const float x_0C = x_00 - x_04;
		const float x_0D = x_01 - x_03;
		const float x_0E = v_B * (x_09 - x_0B);
		const float x_0F = v_B * (x_0C + x_0D);
		const float x_10 = v_B * (x_0C - x_0D);
		const float x_11 = v_2 * x_06;
		const float x_12 = x_05 + x_07;
		const float x_13 = x_05 - x_07;
		const float x_14 = v_B * (x_11 + x_12);
		const float x_15 = v_B * (x_11 - x_12);
		const float x_16 = v_9 * x_13;
