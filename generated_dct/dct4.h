...
{
	static const float v_0 = 0.270598f;
	static const float v_1 = 0.653281f;
	...
	for (int i = 0; i < 4; i++)
	{
		...
		out[1 * stridea] = v_1 * x_2 + v_0 * x_3;
		out[2 * stridea] = v_2 * (x_0 - x_1);
		out[3 * stridea] = v_0 * x_2 - v_1 * x_3;
...
{
	static const float v_0 =  1.414214f;
	static const float v_1 =  1.306563f;
	static const float v_2 =  0.707107f;
	static const float v_3 =  0.541196f;
	static const float v_4 = -0.541196f;
	static const float v_5 =  0.500000f;
	static const float v_6 =  0.353553f;
	...
	for (int i = 0; i < 4; i++)
	{
		...
		const float x_0 = v_0 * s_0;
		const float x_1 = v_1 * s_1 + v_3 * s_3;
		const float x_2 = v_0 * s_2;
		const float x_3 = -v_3 * s_1 + v_1 * s_3;
		const float x_4 = v_5 * (x_0 - x_2);
		const float x_5 = v_2 * x_3;

		out[0 * stridea] = v_6 * (x_0 + x_2) + v_5 * x_1;
		out[1 * stridea] = v_2 * (x_4 - x_5);
		out[2 * stridea] = v_2 * (x_4 + x_5);
		out[3 * stridea] = v_6 * (x_0 + x_2) - v_5 * x_1;
